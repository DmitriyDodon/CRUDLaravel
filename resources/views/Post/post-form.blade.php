...
@section('content')
    <form method="POST">
        ...
        @if($errors->has('user_id'))
            @foreach($errors->get('user_id') as $user_id)
                <div class="alert alert-danger py-3" role="alert">
                    {{ $user_id }}
                </div>
            @endforeach
        ...
        <div class="form-group py-3">
            ...
            <select class="form-control py-3" name="user_id" id="exampleFormControlSelect1">
                @foreach($users as $user)
                    <option  @if(old('user_id') == $user->id ?? $post->user->id == $user->id ) selected @endif  value="{{ $user->id }}">{{ $user->name }}</option>
                @endforeach
            </select>
        </div>
